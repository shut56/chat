...
import { setPopUpActive } from '../../redux/reducers/secondary'

const ServerBlock = () => {
  const dispatch = useDispatch()
  const serverName = useSelector((s) => s.settings.serverSettings.name)
  return (
    <div className="flex justify-between items-center border-b border-gray-700 shadow text-white text-xl px-4 text-gray-100 h-14">
      <span className="truncate">{serverName}</span>
      <button
        type="button"
        className="focus:outline-none"
        onClick={() => dispatch(setPopUpActive(true))}
      >
